...
const app = createApp({
    ...
    methods: {
        ...
        addInitiative() { this.itemModalTitle = 'Add New iNNitiative'; const schema = APP_SCHEMA.definitions.iNNitiative; this.currentEditType = 'initiative'; this.currentEditId = null; this.itemModalFormFields = this.generateFormFields(schema); const today = new Date().toISOString().split('T')[0]; let newInitData = { iNNitiativeId: appUtils.generateUniqueId('INN'), initiativeDateRegistered: today, iNNitiativeDateRegistered: new Date().toISOString() }; for(const key in schema.properties) { if(schema.properties[key].default !== undefined && newInitData[key] === undefined) { newInitData[key] = schema.properties[key].default; }  if(key === "iNNitiativePhase" && newInitData[key] === undefined) newInitData[key] = (this.appData.program && this.appData.program.programStages && this.appData.program.programStages.length > 0) ? this.appData.program.programStages[0] : (schema.properties.iNNitiativePhase.enum ? schema.properties.iNNitiativePhase.enum[0] : ''); if(key === "iNNitiativeType" && newInitData[key] === undefined) newInitData[key] = (this.appData.program && this.appData.program.programDefaultInitiativeTypes && this.appData.program.programDefaultInitiativeTypes.length > 0) ? this.appData.program.programDefaultInitiativeTypes[0] : (schema.properties.iNNitiativeType.enum ? schema.properties.iNNitiativeType.enum[0] : ''); if(key === "initiativeDecision" && newInitData[key] === undefined) newInitData[key] = (schema.properties.initiativeDecision.enum ? schema.properties.initiativeDecision.enum[0] : ''); } this.itemModalFormData = this.prepareFormData(newInitData, this.itemModalFormFields); this.showSaveAndDownload = true; this.itemModalActiveTab = 'edit'; this.showItemModal = true; },
