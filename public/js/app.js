...
import { ViewItemModal } from './components/ViewItemModal.js';
...
const app = createApp({
    ...
    methods: {
        ...
        editProgram() { this.editAddModalTitle = 'Edit Program Configuration'; const schema = APP_SCHEMA.definitions.programConfiguration; this.currentEditType = 'program'; this.currentEditId = null; this.editAddFormFields = this.generateFormFields(schema); if (!this.appData.program || Object.keys(this.appData.program).length === 0) { this.appData.program = { programName: APP_SCHEMA.definitions.programConfiguration.properties.programName.default || "My Innovation Program" }; } this.editAddFormData = this.prepareFormData(this.appData.program, this.editAddFormFields); this.showSaveAndDownload = true; this.showEditAddModal = true; },
        addPerson() { this.editAddModalTitle = 'Add New Person'; const schema = APP_SCHEMA.definitions.person; this.currentEditType = 'person'; this.currentEditId = null; this.editAddFormFields = this.generateFormFields(schema); let newPersonData = { personId: appUtils.generateUniqueId('PERSON') }; for(const key in schema.properties) { if(schema.properties[key].default !== undefined && newPersonData[key] === undefined) { newPersonData[key] = schema.properties[key].default; } } this.editAddFormData = this.prepareFormData(newPersonData, this.editAddFormFields); this.showSaveAndDownload = true; this.showEditAddModal = true; },
        editPerson(person) { this.editAddModalTitle = 'Edit Person'; const schema = APP_SCHEMA.definitions.person; this.currentEditType = 'person'; this.currentEditId = person.personId; this.editAddFormFields = this.generateFormFields(schema); this.editAddFormData = this.prepareFormData(person, this.editAddFormFields); this.showSaveAndDownload = true; this.showEditAddModal = true; },
        ...
        addOpportunity() { this.editAddModalTitle = 'Add New Opportunity'; const schema = APP_SCHEMA.definitions.opportunity; this.currentEditType = 'opportunity'; this.currentEditId = null; this.editAddFormFields = this.generateFormFields(schema); const today = new Date().toISOString().split('T')[0]; let newOppData = { opportunityId: appUtils.generateUniqueId('OPP'), opportunityDateIdentified: today, opportunityLastUpdated: new Date().toISOString() }; for(const key in schema.properties) { if(schema.properties[key].default !== undefined && newOppData[key] === undefined) { newOppData[key] = schema.properties[key].default; } if(key === "opportunityPriority" && newOppData[key] === undefined) newOppData[key] = 0; if(key === "opportunityStatus" && newOppData[key] === undefined) newOppData[key] = (this.appData.program && this.appData.program.programDefaultOpportunityStatuses && this.appData.program.programDefaultOpportunityStatuses.length > 0) ? this.appData.program.programDefaultOpportunityStatuses[0] : (schema.properties.opportunityStatus.enum ? schema.properties.opportunityStatus.enum[0] : ''); } this.editAddFormData = this.prepareFormData(newOppData, this.editAddFormFields); this.showSaveAndDownload = true; this.showEditAddModal = true; },
        editOpportunity(opportunity) { this.editAddModalTitle = 'Edit Opportunity'; const schema = APP_SCHEMA.definitions.opportunity; this.currentEditType = 'opportunity'; this.currentEditId = opportunity.opportunityId; this.editAddFormFields = this.generateFormFields(schema); this.editAddFormData = this.prepareFormData(opportunity, this.editAddFormFields); this.showSaveAndDownload = true; this.showEditAddModal = true; },
        ...
        closeEditAddModal() { this.showEditAddModal = false; this.editAddModalTitle = ''; this.editAddFormData = {}; this.currentEditType = null; this.currentEditId = null; this.showSaveAndDownload = false; this.editAddFormFields = []; },
