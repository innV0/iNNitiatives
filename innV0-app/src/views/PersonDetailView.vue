<template>
  <div class="p-4">
    <div v-if="dataStore.loading.value">Loading person details...</div>
    <div v-else-if="dataStore.error.value">Error loading data: {{ dataStore.error.value }}</div>
    <div v-else-if="personSchema && personData">
      <h1 class="text-2xl font-bold mb-6 text-gray-800">{{ personData.personName || 'Person Details' }}</h1>

      <!-- Back Button/Breadcrumbs -->
      <div class="mb-6">
        <router-link to="/people" class="text-gray-600 hover:text-gray-800 hover:underline transition duration-150 ease-in-out">< Back to People</router-link>
      </div>
                ...
                <div v-if="shouldDisplayInDetail(property)">
                   ...
                   <template v-else-if="property.type === 'object'">
                    ...
                    <template v-else-if="property.format === 'uri' && property.type === 'string'">
                       <a :href="personData[key]" target="_blank" class="text-blue-600 hover:underline">{{ personData[key] || 'N/A' }}</a>
                   </template>
                   <template v-else-if="property.format === 'textarea'">
                       <p class="mt-1 text-gray-700">{{ personData[key] || 'N/A' }}</p>
                   </template>
                   <template v-else>
                      {{ personData[key] || 'N/A' }}
                   </template>
