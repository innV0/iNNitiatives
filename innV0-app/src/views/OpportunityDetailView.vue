<template>
  <div class="container mx-auto p-6">
    <div v-if="dataStore.loading.value" class="text-gray-500 text-center p-4">Loading opportunity details...</div>
    ...
    <div v-else-if="opportunitySchema && opportunityData">
      <h1 class="text-3xl font-bold mb-6 text-gray-800">{{ opportunityData.opportunityName || 'Opportunity Details' }}</h1>

      <!-- Back Button -->
      <div class="mb-6">
        <router-link to="/opportunities" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold py-2 px-4 rounded-md transition duration-150 ease-in-out text-sm">< Back to Opportunities</router-link>
      </div>
                ...
                <div v-if="shouldDisplayInDetail(property)">
                   ...
                   <template v-else-if="property.type === 'object'">
                    ...
                    <template v-else-if="property.format === 'uri' && property.type === 'string'">
                       <a :href="opportunityData[key]" target="_blank" class="text-blue-600 hover:underline ml-2">{{ opportunityData[key] || 'N/A' }}</a>
                   </template>
                   <template v-else-if="property.format === 'textarea'">
                       <p class="mt-1 text-gray-700 ml-2">{{ opportunityData[key] || 'N/A' }}</p>
                   </template>
                   <template v-else>
                      <span class="ml-2 text-gray-700">{{ opportunityData[key] || 'N/A' }}</span>
                   </template>
                </div>
             </div>
           </div>
